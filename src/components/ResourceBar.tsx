...
function TradeItem({ Icon, label, value }: TradeItemProps) {
  return (
    <div className="res-trade-item" title={label}>
      <span className="res-trade-icon"><Icon /></span>
      <span className="res-trade-value">{value.toLocaleString()}</span>
    </div>
  )
}
...
function LuxBadge({ name, has }: LuxBadgeProps) {
  return (
    <span className={`res-lux-badge ${has ? 'res-lux-owned' : 'res-lux-absent'}`} title={name}>
      {name}
    </span>
  )
}
...
export function ResourceBar({ resources }: ResourceBarProps) {
  ...
  return (
    <div className="resource-bar">
      ...
      {/* Divider */}
      <span className="res-bar-divider">✦</span>

      {/* Luxury resources */}
      <div className="res-lux-group">
